...
const hoverClass =
  'transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-maroon/20';
...
export default function PatientFeedback() {
  ...
  return (
    <section
      id="feedback"
      className="scroll-mt-20 bg-white py-10 xs:py-12 sm:py-14 lg:py-16"
    >
      <div className="mx-auto w-full max-w-6xl px-4 xs:px-5 sm:px-7 lg:px-10">
        <div className="mb-7 flex flex-col items-center text-center sm:mb-8">
          ...
          <Link
            to="/feedback"
            className={`inline-flex items-center gap-2 rounded-full bg-maroon px-4 py-2.5 text-[clamp(0.7rem,1.6vw,0.75rem)] font-semibold text-white ${hoverClass}`}
          >
            {feedback.share}
          </Link>
        </div>
          ...
          <StateMessage
            ...
            action={
              <button
                type="button"
                onClick={refetch}
                className={`rounded-full border border-line px-4 py-2 text-[clamp(0.75rem,1.6vw,0.875rem)] font-semibold text-maroon ${hoverClass}`}
              >
                {feedback.retry}
              </button>
            }
