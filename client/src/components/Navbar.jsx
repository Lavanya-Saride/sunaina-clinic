      ...
      >
        <div className="mx-auto w-full max-w-6xl px-3 xs:px-4 sm:px-7 lg:px-10">
          <div className="flex min-h-[64px] items-center justify-between gap-3 sm:min-h-[74px]">
            ...
            <nav
              className="hidden min-w-0 items-center gap-5 md:flex lg:gap-8"
              aria-label={
                UI_CONTENT.primaryNavigation
              }
            >
              {NAV_LINKS.map((link) => (
                <a
                  key={link.href}
                  href={`/${link.href}`}
                  className="group relative whitespace-nowrap py-1 text-[clamp(0.6rem,1.2vw,0.75rem)] font-medium uppercase tracking-[0.1em] text-ink/80 transition-all duration-300 hover:scale-[1.03] hover:text-maroon"
                >
                  {link.label}
            ...
            <button
              ...
              className="inline-flex min-h-9 min-w-[112px] shrink-0 items-center justify-center gap-1.5 rounded-full bg-maroon px-4 text-[clamp(0.65rem,1.8vw,0.875rem)] font-semibold text-white shadow-card transition-all duration-300 hover:-translate-y-0.5 hover:scale-[1.03] hover:shadow-lg hover:ring-1 hover:ring-maroon/30 xs:min-w-[120px] xs:gap-2 xs:px-4 sm:min-h-10 sm:min-w-[128px] sm:px-5"
            >
              <Phone size={15} />
